...
def _gen_matlab_env(env, **kwargs):
    ...
    env['MATLAB'] = {
        ...
        "MEX_FUNCTION_MAP": os.sep.join([matlab_root, 'extern/lib', matlab_arch,'mexFunction.map'])
    }
...
def _mex_builder(env, target, source, gen_def=False, **kwargs):
    ...
    if platform == "win32":
        ...
        env.AppendUnique(LIBS = ["mex", "mx", 'mat'])
    ...
    env.AppendUnique(CPPDEFINES = ["MATLAB_MEX_FILE"],
                     ...
                     RPATH      = [env['MATLAB']['LIB_DIR']],
                     LINKFLAGS  = '-Wl,'+'--version-script='+env['MATLAB']['MEX_FUNCTION_MAP'])
